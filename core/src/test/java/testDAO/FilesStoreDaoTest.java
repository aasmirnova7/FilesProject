...
public class FilesStoreDaoTest{
    ...
    public void deleteTest(){
        User user = new User("1","vasya", "vasichkin", "123");
        FilesStore fs = new FilesStore("AAA",0, user);
        fsd.save(fs);
        fsd.delete(fsd.findWithFileName("AAA").get(0));
        Assert.assertTrue(fsd.findWithFileName("AAA").isEmpty());
    }
    @Test
    public void mergeFileStoreTest(){
        ...
        FilesStore fs = new FilesStore("AAA",0, user);
        fsd.save(fs);
        FilesStore unexpected = new FilesStore("AAA",0, user);
        fs.setPrivacy(1);
        fsd.mergeFileStore(fs);
        Assert.assertNotEquals(fsd.findWithFileName("AAA").get(0),unexpected);
    }
